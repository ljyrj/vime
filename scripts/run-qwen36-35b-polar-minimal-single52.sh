...
#
# ─── 本变体 = 单机 52 基线(single52)· 派生自 run-qwen36-35b-polar-minimal.sh(feature/lb-proxy)───
# 目的:node-5-52 上把「单机 + 单引擎」基线跑通(非 PD、非 DP)。与 base 仅两处默认差异:
#   1. MASTER_ADDR 88 → 本机 IP(hostname -I 首 = 80.48.5.52);base 的 88 在 52 != CURRENT_IP
#      → 走 Ray worker 分支连不存在的 head:6460 卡死。
#   2. FEAT_LB_PROXY 默认 1:单引擎也走 Python 透传 LB proxy(替 Rust router,保 return_token_ids)绑 :8001。
#      → profile.vime.yaml 的 sglang_router_url :8001 直接对(**不用**改 :15000);避 #3 轨迹饿死。
# 数据路径 base 已对(/home/docker/datasets/op_tasks/op_assets_cudallm_filtered189,52 上存在)。
set -ex
